...
Chapter20_Application::Chapter20_Application(std::string title, int width, int height)
    : ChapterBase(title, width, height)
{
    LOG_INFO("Chapter 20 constructed in STANDALONE mode.");
...
#else
Chapter20_Application::Chapter20_Application()
    : ChapterBase()
{
    LOG_INFO("Chapter 20 constructed in BUNDLED mode.");
    for (int i = 0; i < MAX_LIGHTS; ++i)
    {
        m_pLight[i] = std::make_unique<Light>();
    }
...
#endif
Chapter20_Application::~Chapter20_Application() = default;

void Chapter20_Application::setup()
{
...
void Chapter20_Application::setupScene()
{
...
void Chapter20_Application::setupShaders()
{
    m_Shader = std::make_unique<Base::Shader>();
    m_Shader->loadFromFile("shaders/Chapter20.vert", "shaders/Chapter20.frag");
...
void Chapter20_Application::setupGeometry()
{
    setupCube();
    setupLightCube();
    ...
    try
    {
        m_ourModel = std::make_unique<Base::Model>("models/suzanne.obj");
    }
    catch (const std::exception &e)
    {
        LOG_CRITICAL("Failed to load model: {}", e.what());
    }
}

void Chapter20_Application::setupCamera()
{
...
void Chapter20_Application::setupEventListeners()
{
    auto &app = Base::Application::getInstance();
...
void Chapter20_Application::shutdown()
{
    auto &app = Base::Application::getInstance();
...
static CameraInput gatherInput()
{
    ...
    glm::vec2 joyDir = Base::Application::getInstance().getLeftJoystickDirection();
    if (glm::length(joyDir) > 0.0f)
    {
        frameInput.moveRight += joyDir.x;
        // Invert Y-axis: joystick up should move camera forward
        frameInput.moveForward -= joyDir.y;
    }
#endif
...
void Chapter20_Application::render()
{
    ...
    if (m_ourModel)
    {
        ...
        model = glm::translate(model, m_Position);       // Use the existing ImGui transform controls
        model = glm::scale(model, glm::vec3(m_Scale.x)); // Adjust scale to fit the model
        model = glm::rotate(model, glm::radians(m_RotationEuler.y), glm::vec3(0.0f, 1.0f, 0.0f));

        m_Shader->setMat4("model", model);
        m_ourModel->Draw(*m_Shader);
    }
...
void Chapter20_Application::renderChapterUI()
{
...
void Chapter20_Application::handleInput(float deltaTime) {}

void Chapter20_Application::update(float deltaTime)
{
    bool reloaded = false;
...
void Chapter20_Application::setupCube()
{
...
void Chapter20_Application::setupLightCube()
{
...
void Chapter20_Application::setupCoordinateGuide()
{
    float guideVertices[] = {
...
void Chapter20_Application::drawMouseCapturePopup()
{
...
void Chapter20_Application::rebindShaderUniformBlocks()
{
...
void Chapter20_Application::setupSkybox()
{
...
void Chapter20_Application::renderSkybox()
{
